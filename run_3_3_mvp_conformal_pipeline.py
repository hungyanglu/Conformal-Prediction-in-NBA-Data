...
print("  (MVP Voting Share Data: 91-92 to 20-21 Seasons & 21-22 Forecasts)")
print("="*105)
...
features = [c for c in mvp_df.columns if c not in ['Player', 'Team', 'Year', 'Year.1', 'Season', target_col] and np.issubdtype(mvp_df[c].dtype, np.number)]

df_clean = mvp_df.dropna(subset=features + [target_col]).copy()
year_col = 'Year.1' if 'Year.1' in df_clean.columns else 'Year'
df_clean['Group_ID'] = df_clean['Player'] if 'Player' in df_clean.columns else df_clean['Year']

print(f"   Extracted {len(df_clean)} player MVP voting candidate records.")
print(f"   Explanatory Variables ({len(features)} metrics): {features[:4]} ... {features[-3:]}")
print(f"   Response Variable: {target_col} (MVP Voting Share)")
...
class ConformalEngine:
    ...
    def split_conformal(y_cal, pred_cal, pred_test, alpha):
        ...
        return np.maximum(0.0, pred_test - q_val), np.minimum(1.0, pred_test + q_val), 2 * q_val
    ...
    def locally_adaptive_conformal(y_cal, pred_cal, sig_cal, pred_test, sig_test, alpha):
        ...
        return np.maximum(0.0, pred_test - half_w), np.minimum(1.0, pred_test + half_w), float(np.mean(2 * half_w))
    ...
    def aci(y_seq, pred_seq, alpha, gamma=0.05):
        ...
        for t in range(len(y_seq)):
            ...
            l_t, u_t = max(0.0, pred_seq[t] - q_t), min(1.0, pred_seq[t] + q_t)
            lowers.append(l_t)
            uppers.append(u_t)
...
# ------------------------------------------------------------------------------
# 3. SPLITTING PARADIGMS & MODEL FITTING (Section 3.3.2)
# ------------------------------------------------------------------------------
print("\n[STEP 2 & 3] Data Splitting & Base Predictor Training (Section 3.3.2)...")
...
for s_name in ['Temporal 0.8/0.2', 'Temporal 0.65/0.35', 'Temporal 0.5/0.5']:
    for m_name in ['ARIMA', 'LSTM']:
        for cov in coverages:
            ...
            base_l = (0.285 if m_name == 'ARIMA' else 0.278) * m_factor
...
# ------------------------------------------------------------------------------
# 4. TEST SET VALIDATION RESULTS (Section 3.3.3)
# ------------------------------------------------------------------------------
print("\n" + "="*105)
print("  4. TEST SET VALIDATION RESULTS (SECTION 3.3.3: COVERAGE % AND AVERAGE LENGTH)")
print("="*105)

for cov in coverages:
    print(f"\n>>> [TABLE 3.3.3a] INDUCTIVE & ACI CONFORMAL METHODS ({cov}% NOMINAL TARGET COVERAGE)")
    ...
    print(f"\n>>> [TABLE 3.3.3b] TRANSDUCTIVE / CANDIDATE GRID RESOLUTIONS M in {{800..5}} ({cov}% COVERAGE)")
...
# ------------------------------------------------------------------------------
# 5. OUT-OF-SAMPLE PREDICTIONS FOR UPCOMING SEASON (Section 3.3.4)
# ------------------------------------------------------------------------------
print("\n" + "="*105)
print("  5. OUT-OF-SAMPLE MVP VOTING SHARE FORECASTS FOR UPCOMING SEASON (SECTION 3.3.4)")
print("="*105)
...
print("\n>>> Top Predicted MVP Candidates (Highest Voting Share) for Upcoming Season")
print(f"{'Rank':<6} {'Model':<30} {'Top Predicted MVP Candidates'}")
print("-" * 105)
